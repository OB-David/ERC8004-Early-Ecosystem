'''
Use the Etherscan API to fetch account transaction histories much faster than full-node block scanning.
Remember to set the ETHERSCAN_API_KEY environment variable before running this script, or include it in a .env file.
Also your Ethereum node should be provided.
'''
...
def run_transaction_stage(
    ...
    used_etherscan = scan_agent_wallet_transactions_with_etherscan(
        transactions_by_key,
        identity_records,
        signature_map,
        config,
    ...
    if not used_etherscan:
        print(
            "[transactions] Etherscan disabled or missing key; "
            "falling back to ERC-8004 contract-log transactions only"
        )
        scan_agent_wallet_contract_operations(
            transactions_by_key,
            identity_records,
            signature_map,
            config,
